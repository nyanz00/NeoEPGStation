...
export default class TsInfoAnalyzer implements ITsInfoAnalyzer {
    ...
    public async analyze(filePath: string, option?: TsInfoAnalyzeOption): Promise<TsInfo> {
        ...
        const result = await this.scan(
            ...
            maxReadBytes,
            timeoutMs,
            option?.expectedServiceId,
            option?.includeEit !== false,
        );
        ...
        return new Promise<ScanResult>(resolve => {
            ...
            const finish = (): void => {
                ...
                if (info.serviceId !== null) {
                    info.pmtPid = pmtPids.get(info.serviceId) ?? null;
                    // 対象サービスの SDT 情報を反映する
            ...
            tsSectionParser.on('pat', (section: aribts.TsSectionProgramAssociation) => {
                try {
                    ...
                    for (const program of pat.programs) {
                        if (program.program_number !== 0 && typeof program.program_map_PID === 'number') {
                            pmtPids.set(program.program_number, program.program_map_PID);
                        }
                    }
                } catch (err: any) {
                    // 壊れたセクションは無視して読み進める
    ...
    private static hasEnoughInfo(
        ...
        return (
            sdtServices.has(serviceId) &&
            pmtStreams.has(serviceId) &&
            (includeEit === false || eitCandidates.has(serviceId))
        );
    }
