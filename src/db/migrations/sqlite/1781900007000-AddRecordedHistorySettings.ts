...
export class AddRecordedHistorySettings1781900007000 implements MigrationInterface {
    name = 'AddRecordedHistorySettings1781900007000';

    public async up(queryRunner: QueryRunner): Promise<void> {
        if (!(await queryRunner.hasColumn('tv_user', 'isRecordedHistoryEnabled'))) {
            await queryRunner.query(
                'ALTER TABLE "tv_user" ADD "isRecordedHistoryEnabled" boolean NOT NULL DEFAULT (1)',
            );
        }
        if (!(await queryRunner.hasColumn('recorded_playback', 'historyUpdatedAt'))) {
            await queryRunner.query('ALTER TABLE "recorded_playback" ADD "historyUpdatedAt" bigint');
