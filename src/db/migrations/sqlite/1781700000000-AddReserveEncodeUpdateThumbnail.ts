...
export class AddReserveEncodeUpdateThumbnail1781700000000 implements MigrationInterface {
    name = 'AddReserveEncodeUpdateThumbnail1781700000000';

    public async up(queryRunner: QueryRunner): Promise<void> {
        if ((await queryRunner.hasColumn('rule', 'updateThumbnail')) === false) {
            await queryRunner.query(`ALTER TABLE "rule" ADD "updateThumbnail" boolean NOT NULL DEFAULT (0)`);
        }
        if ((await queryRunner.hasColumn('reserve', 'updateThumbnail')) === false) {
            await queryRunner.query(`ALTER TABLE "reserve" ADD "updateThumbnail" boolean NOT NULL DEFAULT (0)`);
        }
    }
