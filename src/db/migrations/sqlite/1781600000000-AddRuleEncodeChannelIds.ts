...
export class AddRuleEncodeChannelIds1781600000000 implements MigrationInterface {
    name = 'AddRuleEncodeChannelIds1781600000000';

    public async up(queryRunner: QueryRunner): Promise<void> {
        if ((await queryRunner.hasColumn('rule', 'encodeChannelId1')) === false) {
            await queryRunner.query(`ALTER TABLE "rule" ADD "encodeChannelId1" bigint`);
        }
        if ((await queryRunner.hasColumn('rule', 'encodeChannelId2')) === false) {
            await queryRunner.query(`ALTER TABLE "rule" ADD "encodeChannelId2" bigint`);
        }
        if ((await queryRunner.hasColumn('rule', 'encodeChannelId3')) === false) {
            await queryRunner.query(`ALTER TABLE "rule" ADD "encodeChannelId3" bigint`);
        }
    }
