...
export class AddUsers1781800000001 implements MigrationInterface {
    name = 'AddUsers1781800000001';

    public async up(queryRunner: QueryRunner): Promise<void> {
        if ((await queryRunner.hasTable('tv_user')) === false) {
            await queryRunner.query(
                'CREATE TABLE `tv_user` (`id` int NOT NULL AUTO_INCREMENT, `name` text NOT NULL, `createdAt` bigint NOT NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB',
            );
        }
        ...
        if ((await queryRunner.hasColumn('rule', 'userId')) === false) {
            await queryRunner.query('ALTER TABLE `rule` ADD `userId` int NULL');
        }
        if ((await queryRunner.hasColumn('reserve', 'userId')) === false) {
            await queryRunner.query('ALTER TABLE `reserve` ADD `userId` int NULL');
        }
        if ((await queryRunner.hasColumn('recorded', 'userId')) === false) {
            await queryRunner.query('ALTER TABLE `recorded` ADD `userId` int NULL');
        }
